...
public class MARS extends AbstractClassifier implements
    ...
    protected double[] doRegression(boolean[] selectedAttributes)
        ...
        no.uib.cipr.matrix.Matrix independentTransposed = null;
        no.uib.cipr.matrix.Matrix independent = null;
        no.uib.cipr.matrix.Vector dependent = null;
        if (numAttributes > 0) {
        ...
        if (numAttributes > 0) {
            ...
            if (!m_useQRDecomposition) { // Use Cholesky based on covariance matrix
                no.uib.cipr.matrix.Vector aTy = independentTransposed.mult(dependent, new DenseVector(numAttributes));
                no.uib.cipr.matrix.Matrix aTa = new UpperSPDDenseMatrix(numAttributes).rank1(independentTransposed);
                ...
                no.uib.cipr.matrix.Vector coeffsWithoutIntercept = aTa.solve(aTy, new DenseVector(numAttributes));
                ...
                independent = null;
                no.uib.cipr.matrix.Matrix Q = qrp.getQ();
                no.uib.cipr.matrix.Matrix R = new UpperTriangDenseMatrix(qrp.getR(), false);
                Matrix P = qrp.getP();
                DenseVector cPlusd = (DenseVector)Q.transMult(dependent, new DenseVector(dependent.size()));
                dependent = null;
                no.uib.cipr.matrix.Vector c = new DenseVector(Arrays.copyOf(cPlusd.getData(), numAttributes));
                no.uib.cipr.matrix.Vector y = R.solve(c, new DenseVector(numAttributes));
                no.uib.cipr.matrix.Vector coeffsWithoutIntercept = P.mult(y, new DenseVector(numAttributes));
                System.arraycopy(((DenseVector) coeffsWithoutIntercept).getData(), 0, coefficients, 0, numAttributes);
            }
        }
